...
#    predictions = Prediction.all().fetch(1000)
    predictions = Prediction.get_user_predictions(self.current_user)
    for prediction in predictions:
      self.response.out.write(db.to_dict(prediction))
...
class PredHandler(BaseHandler):
  def post(self):
    if self.current_user:
      ...
      pred.match = Match.get_by_key_name(self.request.get('match_key'))
      ...
      pred_key = pred.put()
      self.response.out.write('<html><body>pred saved with key -- <pre>')
      self.response.out.write((pred_key))
